#include <stdio.h>
#include <math.h>
#include "../include/fft.h"
#include "../include/cordic.h"

int bit_rev(int num,int N){
	int rev=0;
	while(N!=1){
		rev = (rev<<1)|(num&1);
		num = num>>1;
		N=N/2;
	}
	return rev;
}

void fft_Npt(int *input, double *X_out, double *Y_out,int N)
{
	int i,j,k;
	int offset=0;
	int multiplier=1;
	double theta = 2*pi/N;
	double X_in[1024],Y_in[1024];
	double X_temp[3],Y_temp[3];
	for(i=0;i<N;i++){
		X_in[i]=(double)input[i];
		Y_in[i]=0;
	}

	for(i=N/2;i>=1;i=i/2){
		offset = 0;
		for(j=0;j<N/(i*2);j++){
			for(k=0;k<i;k++){
				X_temp[0] = X_in[k+offset]+X_in[k+i+offset];
				Y_temp[0] = Y_in[k+offset]+Y_in[k+i+offset];

				X_temp[1] = X_in[k+offset]-X_in[k+i+offset];
				Y_temp[1] = Y_in[k+offset]-Y_in[k+i+offset];

				X_temp[2] = (X_temp[1]*cos(theta*multiplier*k)) + (Y_temp[1]*sin(theta*multiplier*k));
				Y_temp[2] = (-(X_temp[1]*sin(theta*multiplier*k))) + (Y_temp[1]*cos(theta*multiplier*k));
				
				X_in[k+offset] = X_temp[0];
				Y_in[k+offset] = Y_temp[0];
				X_in[k+i+offset] = X_temp[2];
				Y_in[k+i+offset] = Y_temp[2];
			}
			offset +=i*2;
		}
		multiplier = multiplier*2;
		printf("---------\n");
	}
	for(i=0;i<N;i++){
		j = bit_rev(i,N);
		X_out[i] = X_in[j];
		Y_out[i] = Y_in[j];
	}
}
